...
import { getUniqueId } from '../utils/chatutils';
...
router.post('/user/create', async (req: Request, res: Response) => {
  ...
  } else if ((userPass as string).length < 6) {
  ...
  if (error !== null) {
    ...
    // No user exists, lets create it!
      ...
      bcrypt.genSalt(10, (err, salt) => {
        bcrypt.hash(userPass as string, salt, (err, hash) => {
          if (err) throw err;
          userPass = hash;
          var date = new Date();
          // Create user
...
router.get('/user/login', async (req: Request, res: Response) => {
  ...
  const isMatch = await bcrypt.compare(userPass as  string, hashPass);
  if (isMatch) {
