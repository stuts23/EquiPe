...
router.post('/channel/create', async (req: Request, res: Response) => {
  ...
  const serverName = (server as string).split('0')[0];
  const serverId = (server as string).split('-')[1];
  if (!channelName || !server || !userId) {
    res.status(400).send('Invalid params');
  } else {
    // Check if user exists
    if (await userIsAdmin(userId as string, serverId)) {
      const channelId = await getUniqueId('channel');
      createChannel(channelId, channelName as string, serverId, userId as string);
...
router.post('/channel/rename', async (req: Request, res: Response) => {
  ...
  if (!channelName || !channelId || !userId || !serverId) {
    ...
    if (await userIsAdmin(userId as string, serverId as string)) {
      renameChannel(channelName as string, channelId as string);
...
router.delete('/channel/delete', async (req: Request, res: Response) => {
  ...
  if (!channelId || !userId) {
    ...
    // Check if user admin
    if (await userIsAdmin(userId as string, serverId as string)) {
      deleteChannel(channelId as string);
