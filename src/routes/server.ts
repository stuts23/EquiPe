...
import { userExists, getUniqueId, userIsAdmin } from '../utils/chatutils';
...
router.post('/server/create', async (req: Request, res: Response) => {
  ...
  if (!serverName || !userId) {
    ...
    if (await userExists(userId as string)) {
      ...
      createServer(serverId, serverName as string, channelId, userId  as string);
...
router.post('/server/join', async (req: Request, res: Response) => {
  ...
  if (!serverId || !userId) {
    ...
    if (await userExists(userId as string)) {
      const response: any = await joinServer(serverId as string, userId as string);
...
router.post('/server/rename', async (req: Request, res: Response) => {
  ...
  if (!serverName || !serverId || !userId) {
    ...
    if (await userIsAdmin(userId as string, serverId as string)) {
      const response = await renameServer(serverName as string, serverId as string);
...
router.delete('/server/delete', async (req: Request, res: Response) => {
  ...
  if (!serverId || !userId) {
    ...
    if (await userIsAdmin(userId as string, serverId as string)) {
      const response = await deleteServer(serverId as string);
...
router.get('/server/admin', async (req: Request, res: Response) => {
  ...
  const response = await userIsAdmin(userId as string, serverId as string);
  res.send(response);
});
...
router.get('/server/activeusers', async (req: Request, res: Response) => {
  ...
  const response = await getActiveUsers(serverId as string);
  res.send(response);
});
